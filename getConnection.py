...
def get_connection(org, auth_token, version, connectionId):
    base_url = f'https://{org}/services/data/{version}/ssot/connections/{connectionId}'
    
    print(f"URL for getting connection: {base_url}")
    ...
    if req.status_code == 200:
        ...
        with open('./sample-responses/getConnection-response.json', 'w') as f:
            json.dump(req.json(), f, indent=2)

        print("Response has been saved to getConnection-response.json")
