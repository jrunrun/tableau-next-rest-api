...
def create_data_stream(org, auth_token, version, stream_data, name):
    url = f'https://{org}/services/data/{version}/ssot/data-streams'

    print(f"URL for creating data stream: {url}")

    # Get dataLakeObjectInfo and remove dataLakeFieldInfoRepresentation
    data_lake_object_info = stream_data.get("dataLakeObjectInfo", {})
    if isinstance(data_lake_object_info, dict):
        # Remove labels from dataLakeFieldInfoRepresentation before removing it
        if "dataLakeFieldInfoRepresentation" in data_lake_object_info:
            for field in data_lake_object_info["dataLakeFieldInfoRepresentation"]:
                field.pop("label", None)
        data_lake_object_info.pop("dataLakeFieldInfoRepresentation", None)
        # Rename dataSpaceInfo to dataspaceInfo
        if "dataSpaceInfo" in data_lake_object_info:
            data_lake_object_info["dataspaceInfo"] = data_lake_object_info.pop("dataSpaceInfo")
        data_lake_object_info.pop("id", None)
        data_lake_object_info.pop("status", None)
        data_lake_object_info.pop("label", None)
        # Remove label from fields
        if "fields" in data_lake_object_info:
            for field in data_lake_object_info["fields"]:
                field.pop("label", None)
        # Remove label from dataLakeObjectInfo itself
        data_lake_object_info.pop("label", None)
        # data_lake_object_info.pop("status", None)

    # Fix datatype to dataType in sourceFields
    source_fields = stream_data.get("sourceFields", [])
    for field in source_fields:
        if "datatype" in field:
            field["dataType"] = field.pop("datatype")

    # Map required fields from existing stream
    # payload = {
    #     "dataLakeObjectInfo": data_lake_object_info,
    #     "name": f"{name} Justin",
    #     "datastreamType": stream_data.get("datastreamType", ""),
    #     "datasource": stream_data.get("datasource", ""),
    #     "sourceFields": source_fields,
    #     "connectorInfo": stream_data.get("connectorInfo", {}),
    #     "refreshConfig": stream_data.get("refreshConfig", {}),
    #     "label": stream_data.get("label", ""),
    #     "mappings": stream_data.get("mappings", [{}])
    # }
